...
public class DrsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private List<String> listaDrs;
    private ListView listViewDrs;
    private List<SESSP> listaSessp,listaEnviar;
    private AlertDialog.Builder alerta;
    private Button buttonProximoDrs,buttonBuscarDrs,buttonAplicarDrs;
    private SESSP sessp;
    private EditText editTextDrs;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //inicia objetos
        listViewDrs = (ListView) findViewById(R.id.listViewDrs);
        buttonProximoDrs = (Button) findViewById(R.id.buttonProximoDrs);
        buttonBuscarDrs = (Button) findViewById(R.id.buttonBuscarDrs);
        editTextDrs = (EditText) findViewById(R.id.editTextDrs);
        buttonAplicarDrs = findViewById(R.id.buttonAplicarDrs);
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, listaDrs);

        listViewDrs.setAdapter(adapter);

        //ação ao clicar na lista
        listViewDrs.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {

                sessp.setDrs(listViewDrs.getItemAtPosition(i).toString());

            }
        });

        //ação aos botoes
        buttonProximoDrs.setOnClickListener(this);
        buttonBuscarDrs.setOnClickListener(this);
        buttonAplicarDrs.setOnClickListener(this);
    }
    ...
    public void montaLista(List<SESSP> lista){
        ...
        for (int a = 0; a<lista.size();a++){
            ...
            listaDrs.add(lista.get(a).getDrs());

        }

        set.addAll(listaDrs);

        listaDrs = new ArrayList<>(set);
    ...
    public List<String> buscarLista(String buscar){
        ...
        int a = 0;
        for (String drs : listaDrs){

            if (drs.contains(buscar)) {
                lista.add(drs);
            }
            a++;
        }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()){
            ...
            case R.id.buttonBuscarDrs:

                sessp.setDrs(null);

                List<String> resultado =  buscarLista(editTextDrs.getText().toString());
            ...
            case R.id.buttonProximoDrs:
                ...
                if(sessp.getDrs()!=null) {

                    alerta.setMessage(this.getString(R.string.drsmensagem) + sessp.getDrs() + "\n" + this.getString(R.string.prosseguir));

                    listaEnviar = new ArrayList<>();

                    for (int a = 0; a<listaSessp.size();a++){

                        if(listaSessp.get(a).getDrs().equals(sessp.getDrs())){
